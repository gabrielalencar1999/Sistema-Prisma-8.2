...
?>
              <div class="modal-body">                                 
                                               ...
                                               <div class="row">
                                                         <div class="col-xs-6">
                                                             ...
                                                             <select class="form-control" id="_tipopessoa" name="_tipopessoa" onchange="_ccampofant()">
                                                                     <option value="1" <?php if($_retorno->{'Tipo_Pessoa'} == 1){ ?>selected <?php } ?>>Física</option>
                                                                     <option value = "2" <?php if($_retorno->{'Tipo_Pessoa'} == 2){ ?>selected <?php } ?>>Jurídica</option>  
                                                              </select>  
                                                          </div>  
                                                          <div class="col-xs-6">
                                                             ...
                                                             <select class="form-control" id="_tipopcliente" name="_tipopcliente" >
                                                               <?php 
                                                               $TIPOCLIENTE = $_retorno->{'TIPO_CLIENTE'};
                                                               if($TIPOCLIENTE == "") {
                                                                      $TIPOCLIENTE = 1;
                                                               }
                                                                      $sqlTipo = "Select * from " . $_SESSION['BASE'] . ".tipocliente ";
                                                                      $consultaTipo = $pdo->query($sqlTipo);                                             
                                                                      if ($consultaTipo->rowCount() > 1){    
                                                                             $retornoEmp = $consultaTipo->fetchAll(\PDO::FETCH_OBJ);                                                                        
                                                                             foreach ($retornoEmp as $rowEmp) {
                                                                           ?>
                                                                           <option value="<?=$rowEmp->tipo_CODIGO_TIPO;?>" <?php if ($rowEmp->tipo_CODIGO_TIPO ==  $TIPOCLIENTE) { ?>selected="selected" <?php } ?> ><?=$rowEmp->tipo_DESCRICAO;?></option>        
                                                                            <?php
                                                                           }
                                                                      }
                                                                      ?>
                                                   ...
                                                   <input type="text" class="form-control" name="_nome" id="_nome" value="<?=$_retorno->{'Nome_Consumidor'};?>">
                                                        ...
                                                        <div class="col-sm-3">
                                                         ...
                                                         <div class="col-sm-3">
                                                            ...
                                                            <input type="text" class="form-control" name="_rgie" id="_rgie"  value="<?=$_retorno->{'INSCR_ESTADUAL'};?>" maxlength="20">
                                                        </div> 
                                                  </div>
                                                  ...
                                                  <div class="row">                                                   
                                                         <div class="col-xs-3">
                                                               <label class="control-label">Telefone Celular 1</label>  
                                                                      <span id="spanfone1" name="spanfone1" class="badge " style="cursor:pointer ;<?php if( $id_celularwats == 0){ echo 'background-color:#79898f'; }else{ echo 'background-color:#81c868';}?>" onblur="validarTefone()" onclick="_atwats('id_celularwats','1','spanfone1')"><i class="fa  fa-whatsapp fa-2"></i></span>   
                                                                      <span id="spanfone2" class="badge " style="cursor:pointer; <?php if($_retorno->{'id_celularsms'} == 0){ echo 'background-color:#79898f'; }else{ echo 'background-color:#337ab7';}?>" onblur="validarTefone()" onclick="_atwats('id_celularsms','2','spanfone2')"><i class="fa  fa-envelope fa-2"></i></span>   
                                                               <input type="text" class="form-control" name="_fonecelular" id="_fonecelular" value="(<?=$_retorno->{'DDD'};?>)<?=$_retorno->{'FONE_CELULAR'};?>" maxlength="14" onblur="validarTefone()" onKeyUp="mascaraTexto(event,'2')">
                                                         </div>
                                                         <div class="col-xs-3">
                                                               <label class="control-label">Celular 2</label>  
                                                                      <span id="spanfone3" class="badge " style="cursor:pointer ;<?php if($_retorno->{'id_celular2wats'} == 0){ echo 'background-color:#79898f'; }else{ echo 'background-color:#81c868"';}?>" onblur="validarTefone()" onclick="_atwats('id_celular2wats','1','spanfone3')"><i class="fa  fa-whatsapp fa-2"></i></span>   
                                                                      <span id="spanfone4" class="badge " style="cursor:pointer ;<?php if($_retorno->{'id_celular2sms'} == 0){ echo 'background-color:#79898f'; }else{ echo 'background-color:#337ab7"';}?>"  onblur="validarTefone()" onclick="_atwats('id_celular2sms','2','spanfone4')"><i class="fa  fa-envelope fa-2"></i></span>   
                                                               <input type="text" class="form-control" name="_fonecelular2" id="_fonecelular2" value="(<?=$_retorno->{'DDD_COM'};?>)<?=$_retorno->{'FONE_COMERCIAL'};?>" maxlength="14" onblur="validarTefone()" onKeyUp="mascaraTexto(event,'2')">
                                                         </div>
                                                         <div class="col-xs-6">
                                                               <label class="control-label">Telefone Fixo</label>    
                                                               <input type="text" class="form-control" name="_fonefixo" id="_fonefixo" value="(<?=$_retorno->{'DDD_RES'};?>)<?=$_retorno->{'FONE_RESIDENCIAL'};?>" maxlength="14" onblur="validarTefone()" onKeyUp="mascaraTexto(event,'3')">
                                                         </div>
                                                  </div>
                                                  ...
                                                  <div class="row">
                                                        ...
                                                        <div class="col-sm-6">
                                                               ...
                                                               <?php
                                                               $query = ("SELECT usuario_CODIGOUSUARIO,usuario_APELIDO  FROM usuario  where usuario_tecnico = '1'");
                                          ...
                                          <div class="modal-footer">
                                                 <?php    if ($_SESSION['per163'] == '163') {//ACESSO bloqueia alteracao cliente
                                                         } else { ?>
                                             <button type="button" class="btn btn-success waves-effect"  id="_bt000008"  onclick="_000008()" ><i class="fa  fa-save"></i> Salvar</button>
                                                 <?php } ?>
                                             <button type="button" class="btn btn-inverse waves-effect" data-dismiss="modal"><i class="fa fa fa-times"></i>Fechar</button>
