...
if ($_acao == 8) {
    ...
    $situacaooficina = $_parametros['_idstatustrack'];
    $sql = "Select * from " . $_SESSION['BASE'] . ".situacao_oficina where sitmobOF_id = '$situacaooficina'"; 
    $stm = $pdo->prepare("$sql");                     
    $stm->execute();                                                             
    if ($stm->rowCount() > 0) {                                                             
        while ($linha = $stm->fetch(PDO::FETCH_OBJ)) // retornar os dados em formato de objeto
    ...
    ?>
        <div class="row">
            <div class="col-sm-12 col-xs-12" style="text-align:center ; padding-top:50px">
                <strong>Deseja realmente <span style="color:<?=$linha->sitmobOF_cortable;?>;">Finalizar Atendimento </span> ? </strong>
            </div>
        </div>
        <div class="row">
            ...
            <div class="col-sm-8 col-xs-8">                
                <button id="cadastrar" type="button" class="btn btn-block btn-lg  waves-effect waves-light mb-auto"   style="height:70px;margin:5px;background-color:<?=$linha->sitmobOF_cortable;?>;color:<?=$linha->sitmobOF_cortfont;?>;" onclick="_selFinalizar('<?=$_parametros['_idstatustrack'];?>')"><i class="ion-checkmark-circled"></i> <?=$linha->sitmobOF_descricao;?></button>      
            </div>
        </div>
        <?php
        }
    ...
    if ($_acao == 9) {
        ...
        if ($stm->rowCount() > 0) {                                                             
            while ($linha = $stm->fetch(PDO::FETCH_OBJ)) // retornar os dados em formato de objeto
            {  
                $_descoficina = $linha->sitmobOF_descricao;
            }
        }
        ...
        if ($_parametros['_motivoselecionado'] == 2) { //orçamento

            $_msgacompanhamento = "<strong>ACOMPANHAMENTO AUTOMÁTICO OFICINA</strong> - $_descoficina";
        ...
        if ($_parametros['_motivoselecionado'] == 1) { //Concluido

            $_msgacompanhamento = "<strong>ACOMPANHAMENTO AUTOMÁTICO OFICINA</strong> - Finalizado Atendimento";
        ...
        TAXA = '" . $vlrtaxa . "',DESC_SERVICO = '" . $vlrdesconto . "',
        DESCRICAO_TAXA = '" . $_parametros['_txdescricao'] . "',      
        SIT_OFICINA  = '" . $_parametros['_idstatustrack'] . "' 
     ...
     /*
        ...
        if ($empresaEnvio > 0) {
            ...
            while ($rst2 = mysqli_fetch_array($result)) {
                ...
                $documento = $OS;
                $consulta = "Select 
                consumidor.CODIGO_CONSUMIDOR as idcli,Nome_Consumidor,Nome_Rua,Num_Rua,
