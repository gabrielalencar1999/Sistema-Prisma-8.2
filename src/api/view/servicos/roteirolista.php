...
<tbody>
    ...
    $_os =  $_parametros['_os'];
     ...
     if($assessor != "" ) { 
        ...
        $sql = "Select sitmob_descricao,g_sigla,CODIGO_CHAMADA,Nome_Consumidor,FONE_CELULAR,FONE_COMERCIAL,FONE_RESIDENCIAL,
        Nome_Rua,Num_Rua,BAIRRO,CIDADE,CEP,COMPLEMENTO,
        Obs_Atend_Externo,situacaoos_elx.DESCRICAO,DDD,
         DATE_FORMAT(DATA_CHAMADA, '%d/%m/%Y' ) AS DATA,
                    ...
                    chamada.DESCRICAO as descproduto,NUM_ORDEM_SERVICO,
        ...
        $numosfabricante = $linha->NUM_ORDEM_SERVICO;	
        ...
        ?>
            ...
            <td style="text-align: center;"><?= $garantia;?></td>
                ...
                <td><?=$linha->tecnico;?></td>              
                <td style="min-width:200px;  white-space: pre-wrap;"><?=$linha->descproduto;?></td>                        
                <td><?=$linha->DATA;?></td>
                <td><?=$linha->DATA_A;?></td>
                <?php
                if($_retviewerRoteiro == 1) {
