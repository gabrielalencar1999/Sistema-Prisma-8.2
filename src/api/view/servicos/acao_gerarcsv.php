...
if($_acao == 1 ) {   //gerar csv roteiro
    ...
    $arquivo_caminho = "docs/".$_SESSION['CODIGOCLI']."/Roteiro.csv";
    if(is_dir($dir))
        {
            //echo "A Pasta Existe";
        }
	   ...
	   if($assessor != "" ) { 
		  ...
		  Obs_Atend_Externo,situacaoos_elx.DESCRICAO,DDD,
		   DATE_FORMAT(DATA_CHAMADA, '%d/%m/%Y' ) AS DATA,
					  ...
					  , A.usuario_APELIDO  as tecnico, B.usuario_APELIDO  as atendente,
					  ...
					  and  ISNULL(DATA_ENCERRAMENTO)  $vend $sit $_filgarantia $_filosAnd or 
					  $filtro between	'$dataini' and '$datafim' and  DATA_ENCERRAMENTO = '0000-00-00' $vend $sit $_filgarantia $_filosAnd $_filos
					  order by $ordem";
	   				 //sitmob_descricao
					  //left join ". $_SESSION['BASE'] .".situacao_trackmob on trackO_situacaoEncerrado = sitmob_id
		   ...
		   // $_itemlinha = "Codigo;Descrição;Qtd;V Unit;V Total;IPI;PICMS;MVA;BC STR;V STR";
			...
			$_itemlinhatitulo = "Nº OS;Garantia;Assessor;Sit O.S;Dt Abertura;Dt Atendimento;Cliente;Telefone;Cep;Endereço;Bairro;Cidade;Observação;Situação;Produto,Modelo,serie,Atendente";
