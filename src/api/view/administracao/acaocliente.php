...
if ($acao == 3) { // lista equipamento ja do cliente
    ...
    group by CODIGO_FABRICANTE,descricao,marca, serie LIMIT 50 order by CODIGO_CHAMADA desc";
...
if ($acao == 8) { // Lista equipamento  OS
	...
	$sql = "Select descricao,Modelo,serie,marca,Revendedor,Data_Nota,Nota_Fiscal,VOLTAGEM,cnpj
			from " . $_SESSION['BASE'] . ".chamada 	left join " . $_SESSION['BASE'] . ".consumidor on chamada.CODIGO_CONSUMIDOR = consumidor.CODIGO_CONSUMIDOR 
			where chamada.CODIGO_CONSUMIDOR = '" . $_parametros['_idcliente'] . "' 
		    group by descricao,marca, serie,Modelo LIMIT 50 order by CODIGO_CHAMADA desc";
...
<div id="dvlista">
	...
	if ($consulta->rowCount() > 0) {
		foreach ($retorno as $row) {
            ...
            $datanf = $row['Data_Nota'];         
            $revendedor =$row['Revendedor'];
            $cnpj = $row['cnpj'];
         
				$equi = $desc.";".$marca.";".$modelo.";".$serie.";".$voltagem.";".$PNC.";".$notafiscal.";".$datanf.";".$voltagem.";".$revendedor.";".$cnpj;  
	?>
