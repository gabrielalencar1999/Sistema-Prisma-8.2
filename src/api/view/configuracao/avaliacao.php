...
.box-categoria{
	...
	box-shadow: 0px 0px 19px 13px rgba(186,186,186,0.22); width:98%; height:98px; float:left; margin-left:10px; margin-bottom:10px; background-color:#FFF; padding:10px; text-align:center;
	border-radius:12px;
}
.addSub{
	color:#00a8e6;
...
<div class="wrapper">
    <div class="container">
        <div class="row">
            <div class="col-xs-6">
                <h4 class="page-title m-t-15">Avaliação</h4>
                <p class="text-muted page-title-alt">Inclua e altere avaliações.</p>
            </div>
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="card-box table-responsive" id="linha-listagem" style="background-color:#EBEFF2; border:0px;">
...
<script type="text/javascript">
	...
	function gerarSintoma(){
		
		var $_keyid =   "avaliacao_00002";
		var dados = $("#form3 :input").serializeArray();
		dados = JSON.stringify(dados);			
		$.post("page_return.php", {_keyform:$_keyid, dados:dados , acao:"insertSintoma"}, function(result){	
			if(result == 1){ }else{
				_lista();
				$("#divAlt").html(
				'<div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button><h4 class="modal-title"></h4></div><div class="modal-body"><div class="col-sm-12" align="center"><p> Sintoma incluído com <strong>Sucesso !!!</strong></p></div><div class="row"><div class="col-sm-12" align="center"><img src="assets/images/small/img_cadastro.jpg" alt="image" class="img-responsive "width="200"/></div></div></div>'
				);	
			}
		});
	}	
	...
	function altSintoma(){
		...
		var dados = $("#form3 :input").serializeArray();
		dados = JSON.stringify(dados);			
		$.post("page_return.php", {_keyform:$_keyid, dados:dados, acao:"alteraSintoma"}, function(result){		
			if(result == 1){ }else{
				_lista();
				$("#divAlt").html(
				'	<div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button><h4 class="modal-title"></h4></div><div class="modal-body"><div class="col-sm-12" align="center"><p> Sintoma alterado com <strong>Sucesso !!!</strong></p></div><div class="row"><div class="col-sm-12" align="center"><img src="assets/images/small/img_cadastro.jpg" alt="image" class="img-responsive " width="200"/></div></div></div>'
				);	
			}
		});
	}
	...
	function excluirSubcat(){
		...
		var dados = $("#form3 :input").serializeArray();
		dados = JSON.stringify(dados);			
		$.post("page_return.php", {_keyform:$_keyid, dados:dados, acao:"excluirAvaliacao"}, function(result){		
			if(result == 1){ }else{
				_lista();
				$("#divAlt").html(
				'	<div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button><h4 class="modal-title"></h4></div><div class="modal-body"><div class="col-sm-12" align="center"><p> Avaliacao deletada com <strong>Sucesso !!!</strong></p></div><div class="row"><div class="col-sm-12" align="center"><img src="assets/images/small/img_cadastro.jpg" alt="image" class="img-responsive " width="200"/></div></div></div>'
				);	
			}
		});
	}	
