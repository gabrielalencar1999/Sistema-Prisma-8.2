...
$sql = "SELECT 
            ...
            fs_acompmedicamento,
            fs_examemedicamento
        FROM " . $_SESSION['BASE'] . ".ficha_saude 
        WHERE fs_CodUsuario = '$id'";

$stm = $pdo->query($sql);
$rst = $stm->fetch();

// Mapeando os valores para variáveis
$tiposanguineo           = $rst["tipo_sanguineo"];
$tipoalergia             = $rst["tipo_alergia"];
$medicamento_controlado  = $rst["medicamento_controlado"];
$medicamento_dor         = $rst["medicamento_dor"];
$problema_saude          = $rst["problema_saude"];   
$nomecontatoemergencia   = $rst["ctt_emergencia_nome"];
$numerocontatoemergencia = $rst["ctt_emergencia_telefone"];
$fs_fumante              = $rst["fs_fumante"];
$convenio                = $rst["fs_convenio"];
$fs_usomedicamento       = $rst["fs_usomedicamento"];
$fs_acompmedicamento     = $rst["fs_acompmedicamento"];
$fs_examemedicamento     = $rst["fs_examemedicamento"];
...
<div class="wrapper">
    <div class="container">
                ...
                <div class="card-box">
                    <div class="panel-body">
                        <ul class="nav nav-pills m-b-30">
                            ...
                            <li class="">
                                <a href="#navpills-51" data-toggle="tab" aria-expanded="false">Ficha de Saúde</a>
                            </li>
                            <li class="">
       ...
       <div id="controle-horario" class="col-md-12" style="display:none; margin-top:20px;">
    
    <div class="card">
        <div class="card-body">
            <div style="border: 2px solid #f0f0f0; padding: 15px; border-radius: 8px;">
            <h4 class="card-title mb-3"><b>Controle de horário - Periodo</b></h4>
            ...
            <div class="row align-items-end">
              ...
              <div class="form-group col-md-2">
    <label for="entradam-usuario">Manhã Entrada</label>
    <input type="time" class="form-control" name="entradam-usuario" id="entradam-usuario" value="<?= $rst['usuario_MANHAENTRADA'] ?>">
</div>

<div class="form-group col-md-2">
    <label for="saidam-usuario">Manhã Saída</label>
    <input type="time" class="form-control" name="saidam-usuario" id="saidam-usuario" value="<?= $rst['usuario_MANHASAIDA'] ?>">
</div>

<div class="form-group col-md-2">
    <label for="entradat-usuario">Tarde Entrada</label>
    <input type="time" class="form-control" name="entradat-usuario" id="entradat-usuario" value="<?= $rst['usuario_TARDEENTRADA'] ?>">
</div>

<div class="form-group col-md-2">
    <label for="saidat-usuario">Tarde Saída</label>
    <input type="time" class="form-control" name="saidat-usuario" id="saidat-usuario" value="<?= $rst['usuario_TARDESAIDA'] ?>">
</div>

<div class="form-group col-md-2">
    <label for="entradas-usuario">Sábado Entrada</label>
    <input type="time" class="form-control" name="entradas-usuario" id="entradas-usuario" value="<?= $rst['usuario_sabadoe'] ?>">
</div>

<div class="form-group col-md-2">
    <label for="saidas-usuario">Sábado Saída</label>
    <input type="time" class="form-control" name="saidas-usuario" id="saidas-usuario" value="<?= $rst['usuario_sabados'] ?>">
</div>
                                                </div>       
                                               </div>
                </div>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        const select = document.getElementById("acessoexterno");
        const bloco = document.getElementById("controle-horario");

        function toggleBloco() {
            if (select.value === "Nao") {
                bloco.style.display = "block"; // mostra se for NÃO
            } else {
                bloco.style.display = "none"; // esconde se for SIM
            }
        }

        // dispara ao carregar a página (pra já respeitar o valor vindo do banco)
        toggleBloco();

        // dispara quando mudar
        select.addEventListener("change", toggleBloco);
    });
</script>
    ...
    <div class="form-group col-md-3">
        ...
        <input type="text" name="usuario_fonecomercial" class="form-control" id="telrecado" value="<?=$rst["usuario_fonecomercial"];?>">
    </div>
    <div class="form-group col-md-3">
        <label for="telrecado">Contato:</label>
        <input type="text" name="usuario_nomerecado" class="form-control" id="usuario_nomerecado" value="<?=$rst["usuario_nomerecado"];?>">
    </div>
</div>
...
<div class="row">
    <div class="col-md-4">
        <div class="form-group">
            <label for="usuario_emaicorp"><b>Email Corporativo:</b></label>
            <input type="text" name="usuario_emaicorp" class="form-control" id="usuario_emaicorp" value="<?=$rst["usuario_emaicorp"];?>">
        </div>
    </div>
    ...
    <div class="col-md-4">
        <div class="form-group">
            <label for="email-usuario">
                <b class="text-danger">Email Pessoal:</b> 
                <code>* importante p/ registro ponto</code>
            </label>
...
<input type="text" name="tiposanguineo-usuario" class="form-control" id="tiposanguineo" 
       value="<?=isset($tiposanguineo)?$tiposanguineo:"";?>">

    <!-- Número do contato -->
    <div class="form-group col-md-6" style="margin-top:15px;">
    <label>Número do contato de emergência:</label>
    <input type="text" name="numerocontatoemergencia-usuario" class="form-control" id="numerocontatoemergencia" 
           placeholder="Número do contato de emergência..." 
           value="<?=isset($numerocontatoemergencia)?$numerocontatoemergencia:"";?>">
    </div>
    
   <!-- Nome do contato -->
<div class="form-group col-md-6" style="margin-top:15px;">
    <label>Nome do contato de emergência:</label>
    <input type="text" name="nomecontatoemergencia-usuario" class="form-control" id="nomecontatoemergencia" 
           placeholder="Nome do contato de emergência..." 
          value="<?=isset($nomecontatoemergencia)?$nomecontatoemergencia:"";?>">        
</div>
</div>
...
<label for="fumante-usuario" class="form-label"><strong>Fumante:</strong></label>
<select id="fumante-usuario" name="fumante-usuario" class="form-control form-control-sm">
   <option value="" <?= ($fs_fumante === null) ? "selected" : "" ?>>Selecione</option>
    <option value="1" <?= ($fs_fumante == 1) ? "selected" : "" ?>>Sim</option>
    <option value="0" <?= ($fs_fumante == 0 && $fs_fumante !== null) ? "selected" : "" ?>>Não</option>
    ...
    <div class="form-group col-md-6">
    <label>Em caso de dor ou febre, qual medicação podemos dar?</label>
    <input type="text" name="medicamento-usuario" class="form-control" id="medicamento" 
           value="<?=isset($medicamento_dor)?$medicamento_dor:"";?>">
</div>
</div>                                 
                    <div class="form-group">
    <div class="checkbox">
        <input type="checkbox" id="possuiAlergia" name="possuiAlergia"
               onchange="document.getElementById('alergiaDescricao').style.display = this.checked ? 'block' : 'none';">
        <label for="possuiAlergia">Possui alguma alergia?</label>
    </div>
    <textarea name="alergia-usuario" id="alergiaDescricao" class="form-control" 
              style="display:none; margin-top:8px;" placeholder="Descreva a alergia..."><?=isset($tipoalergia)?$tipoalergia:"";?></textarea>
</div>
                                        <script>
                                           
                                            window.addEventListener('DOMContentLoaded', function() {
                                                var alergia = document.getElementById('alergiaDescricao');
                                                var checkbox = document.getElementById('possuiAlergia');
                                                if(alergia.value && alergia.value.trim() !== '') {
    ...
    <div class="checkbox">
    <input type="checkbox" id="usoMedicamento" name="usoMedicamento"
               onchange="document.getElementById('usoMedicamentoDescricao').style.display = this.checked ? 'block' : 'none';">
    <label for="usoMedicamento">Faz uso  contínuo de medicamento controlado?</label>
    </div>
    <textarea name="usoMedicamento-usuario" id="usoMedicamentoDescricao" class="form-control"
              style="display:none; margin-top:8px;" placeholder="Nome do medicamento..."><?=isset($medicamento_controlado)?$medicamento_controlado:"";?></textarea>
    </div>

                                        <script>
                                            window.addEventListener('DOMContentLoaded', function () {
                                                var medicamento = document.getElementById('usoMedicamentoDescricao');
                                                var checkbox = document.getElementById('usoMedicamento');

                                                if (medicamento.value && medicamento.value.trim() !== '') {
                                                    checkbox.checked = true;
                                                    medicamento.style.display = 'block';
                                                }
                                            });
                                            // Bloco tipo medicamento
                                        </script>
    ...
    <div class="checkbox">
        <input type="checkbox" id="possuiConvenio" name="possuiConvenio"
            onchange="document.getElementById('convenioDescricao').style.display = this.checked ? 'block' : 'none';">
        <label for="possuiConvenio">Possui algum convênio de saúde ou médico de confiança?</label>
    </div>

    <textarea name="convenio-usuario" id="convenioDescricao" class="form-control"
        style="display:none; margin-top:8px; resize: vertical;" rows="1" placeholder="Nome do médico ou convênio..."><?=$convenio?></textarea>
</div>

<script>
window.addEventListener('DOMContentLoaded', function() {
    var convenio = document.getElementById('convenioDescricao');
    var checkbox = document.getElementById('possuiConvenio');
    if(convenio.value && convenio.value.trim() !== '') {
        checkbox.checked = true;
...
<div class="form-group">
  <div class="checkbox">
    <input type="checkbox" id="historicoSaudedetalhe" name="historicoSaudedetalhe"
 onchange="document.getElementById('historicoSaudeDetalhado').style.display = this.checked ? 'block' : 'none';">
 
    <label for="historicoSaudedetalhe">Tem histórico medico?</label>
  </div>
</div>
      </div>
  ...
  <div id="historicoSaudeDetalhado" style="display:none; margin-top:8px;">
    <div class="form-group">
    <label>Faz uso de medicamentos?</label>
    <select name="usoMedicamentos" class="form-control">
    <option value="" <?= ($fs_usomedicamento === null) ? "selected" : "" ?>>Selecione</option>
<option value="1" <?= ($fs_usomedicamento == 1) ? "selected" : "" ?>>Sim</option>
<option value="0" <?= ($fs_usomedicamento === "0" || $fs_usomedicamento === 0) ? "selected" : "" ?>>Não</option>
    ...
    <div class="form-group">
   <label>Faz algum acompanhamento médico?</label>
<select name="acompanhamentoMedico" class="form-control">
    <option value="" <?= ($fs_acompmedicamento === null) ? "selected" : "" ?>>Selecione</option>
    <option value="1" <?= ($fs_acompmedicamento == 1) ? "selected" : "" ?>>Sim</option>
    <option value="0" <?= ($fs_acompmedicamento == 0 && $fs_acompmedicamento !== null) ? "selected" : "" ?>>Não</option>
</select>
  </div>

    <div class="form-group">
    <label>Faz exames periódicos?</label>
<select name="examesPeriodicos" class="form-control">
    <option value="" <?= ($fs_examemedicamento === null) ? "selected" : "" ?>>Selecione</option>
    <option value="1" <?= ($fs_examemedicamento == 1) ? "selected" : "" ?>>Sim</option>
    <option value="0" <?= ($fs_examemedicamento == 0 && $fs_examemedicamento !== null) ? "selected" : "" ?>>Não</option>
</select>
  </div>

    <div class="form-group">  
  <label>Possui alguma condição abaixo:</label>
  <div class="row">
          </div>
    </div>
    <div class="form-group">
    <div class="checkbox">
        <input type="checkbox" id="problemasCheckbox" name="problemasCheckbox" <?= $checked ?>
               onchange="document.getElementById('problemasSaude-usuario').style.display = this.checked ? 'block' : 'none';">
        <label for="problemasCheckbox">Possui algum desses problemas?</label>
    </div>
    <textarea name="problemasSaude-usuario" id="problemasSaude-usuario" class="form-control" 
              style="display:<?= $display ?>; margin-top:20px; height: 200px;"><?= !empty($problema_saude) ? htmlspecialchars($problema_saude) : " *SUGESTÕES*\nDiabetes\nHipertensão\nProblema Cardíaco\nRefluxo\nProblemas Gástricos\nCirculatórios\nRinite/Sinusite\nEnxaqueca\nCólicas\nCâncer\nHipoglicemia\nProblema Renal\nHipotireoidismo\nHipertireoidismo\nFadiga Crônica" ?></textarea>
</div>
</div>

<script>
    // Se você estiver editando dados, você pode precisar deste script para 
    // garantir que o checkbox e o textarea de alergia apareçam se houver dados.
    window.addEventListener('DOMContentLoaded', function() {
        var alergia = document.getElementById('problemasSaude-usuario');
        var checkbox = document.getElementById('problemasCheckbox');
        
        // Esta parte verifica se a variável de backend (que antes era $tipoalergia) 
        // tem algum valor para marcar a caixa e exibir o campo.
        // Se você não usa variáveis de backend (como PHP), pode ignorar este script.
        // O valor padrão para um novo formulário seria vazio, então isso não afetaria.
        if(alergia.value && alergia.value.trim() !== '') {
            checkbox.checked = true;
            alergia.style.display = 'block';
        }
