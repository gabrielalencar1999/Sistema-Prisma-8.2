...
<div class="wrapper">
    <div class="container">
        <div class="row">
            <div class="col-xs-6">
                <h4 class="page-title m-t-15">Situação de Atendimento</h4>
                <p class="text-muted page-title-alt">Cadastro situação</p>
            </div>
            ...
            <div class="modal-body">
                <form id="form-inclui">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label>Bloqueia OS</label>
                                <select class="form-control" id="addSitelxbloqueia">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                <label>Ativo</label>
                                <select class="form-control" id="addSitelxAtivo">
                                    <option value="1">Sim</option>
                                    <option value="0">Não</option>
                                </select>
                            </div>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                <label>Situação Oficina</label>
                                <select class="form-control" id="addSituacaoOficina">
                                    <option value="">Selecione...</option>
                                    <?php
                                    $sql = "SELECT sitmobOF_id, sitmobOF_descricao FROM bd_novo.situacao_oficina ORDER BY sitmobOF_descricao";
                                    $result = $mysqli->query($sql);
                                    $situacoes = $result->fetch_all(MYSQLI_ASSOC);
                                    foreach ($situacoes as $situacao): ?>
                                    <option value="<?php echo htmlspecialchars($situacao['sitmobOF_id']); ?>">
                                        <?php echo htmlspecialchars($situacao['sitmobOF_descricao']); ?>
                                    </option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="color" class="form-control" id="addCorPersonalizada" value="#ffffff">
                                <button type="button" class="btn btn-outline-secondary btn-block" onclick="document.getElementById('addCorPersonalizada').value = '#ffffff';">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="color" class="form-control" id="addCorFonte" value="#000000">
                                <button type="button" class="btn btn-outline-secondary btn-block" onclick="document.getElementById('addCorFonte').value = '#000000';">
                                    <i class="fa fa-eraser"></i> Limpar
                                </button>
                            </div>
                        </div>
                ...
                <form id="form-altera">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label>Bloqueia OS</label>
                                <select class="form-control" id="editSitelxbloqueia">
                                    <option value="1">Sim</option>
                                    <option value="0">Não</option>
                                </select>
                            </div>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                <label>Situação Oficina</label>
                                <select class="form-control" id="editSituacaoOficina">
                                    <option value="">Selecione...</option>
                                    <?php
                                    $sql = "SELECT sitmobOF_id, sitmobOF_descricao FROM bd_novo.situacao_oficina ORDER BY sitmobOF_descricao";
                                    $result = $mysqli->query($sql);
...
<script type="text/javascript">
    ...
    function _incluir() {
        var $_keyid = "sit00002"; // Chave do formulário para identificar a operação
        
        // Coleta todos os dados do formulário de inclusão
        var dados = {
            descricao: $('#addDescricao').val(), // Descrição da situação
            sitelx_bloqueia: $('#addSitelxbloqueia').val(), // Indica se bloqueia OS (0 ou 1)
            sitelx_ativo: $('#addSitelxAtivo').val(), // Indica se está ativo (0 ou 1)
            situacao_oficina: $('#addSituacaoOficina').val(), // ID da situação oficina selecionada
            cor_sitcodigo: $('#addCorPersonalizada').val(), // Cor de fundo personalizada
            cor_sitcodigofonte: $('#addCorFonte').val() // Cor da fonte
        };
        ...
        $.post("page_return.php", {
            _keyform: $_keyid, // Chave do formulário
    ...
    function _buscadados(id) {
        ...
        $('#id-altera').val(dadosLinha.COD_SITUACAO_OS);
        $('#editDescricao').val(dadosLinha.DESCRICAO);
        $('#editSitelxbloqueia').val(dadosLinha.sitelx_bloqueia);
        $('#editSitelxAtivo').val(dadosLinha.sitelx_ativo);
        $('#editSituacaoOficina').val(dadosLinha.sitmobOF_id);
        $('#editCorPersonalizada').val(dadosLinha.cor_sitcodigo || '#ffffff');
        $('#editCorFonte').val(dadosLinha.cor_sitcodigofonte || '#000000');
    ...
    function _incluir() {
        ...
        var dados = {
            ...
            sitelx_bloqueia: parseInt($('#addSitelxbloqueia').val()) || 0,
            sitelx_ativo: parseInt($('#addSitelxAtivo').val()) || 0,
            COLUNA_PLANILHA: $('#addSituacaoOficina').val(),
            cor_sitcodigo: $('#addCorPersonalizada').val(),
            cor_sitcodigofonte: $('#addCorFonte').val()
        };
    ...
    function _alterar() {
        ...
        var dados = {
            COD_SITUACAO_OS: $('#id-altera').val(), // ID do registro que está sendo editado
            DESCRICAO: $('#editDescricao').val(), // Nova descrição
            sitelx_bloqueia: parseInt($('#editSitelxbloqueia').val()) || 0, // Valor do bloqueio de OS (converte para número)
            sitelx_ativo: parseInt($('#editSitelxAtivo').val()) || 0, // Valor do campo ativo (converte para número)
            COLUNA_PLANILHA: $('#editSituacaoOficina').val(), // ID da situação oficina selecionada
            cor_sitcodigo: $('#editCorPersonalizada').val(), // Nova cor personalizada
            cor_sitcodigofonte: $('#editCorFonte').val(), // Nova cor da fonte
            sitmobOF_id: $('#editSituacaoOficina').val() // ID da situação oficina novamente (para garantir consistência)
        }
        
        if (!dados.COD_SITUACAO_OS) {
    ...
    $.ajax({
            ...
            data: {
                _keyform: "sit00002", // Chave do formulário
                dados: JSON.stringify(dados), // Dados do formulário em formato JSON
    ...
    function _abrirModalAlterar(dados) {
        ...
        $('#id-altera').val(dados.COD_SITUACAO_OS);
        $('#editDescricao').val(dados.DESCRICAO);
        $('#editSitelxbloqueia').val(dados.sitelx_bloqueia);
        $('#editSitelxAtivo').val(dados.sitelx_ativo);
        
        // Preenche o select de situação oficina e verifica se está funcionando
        $('#editSituacaoOficina').val(dados.sitmobOF_id);
        
        $('#editCorPersonalizada').val(dados.cor_sitcodigo || '#ffffff');
        $('#editCorFonte').val(dados.cor_sitcodigofonte || '#000000');
        $('#custom-modal-alterar').modal('show');
    }
    ...
    function excluirSub() { // referencia a função onclick do botão excluir do modal
        var $_keyid = "sit00002";  // referencia ao arquivo acesso que passa a rota para o arquivo page_return.php
        $('#_keyform').val($_keyid);
        var dados = $("#form1 :input").serializeArray();
        dados = JSON.stringify(dados);
