...
/*
                        ...
                        */
                        ...
                        $data = strtotime("$ano-$mes-$dia " . date("H:i:s"));
         ?>
<!DOCTYPE html>
<html>
<head>
               ...
               //   $_SESSION["login"] = $login; 
                //  if ($numrows!=0){		//2
                  ...
                  //  $rst_sel =  $result->fetch_assoc();
                     ...
                     // Inicializa variáveis de horário
                    $A = $B = $C = $D = $E = $F = null;
/*
                    if( $mEntrada != "") {               
                     ...
                     */
                     $mEntrada = $rst_sel['usuario_MANHAENTRADA']; // ex: 08:30:00
                   
                        $A = strtotime("$ano-$mes-$dia $mEntrada");

                        $mSaida = $rst_sel['usuario_MANHASAIDA'];
                        $B = strtotime("$ano-$mes-$dia $mSaida");

                        $tEntrada = $rst_sel['usuario_TARDEENTRADA'];
                        $C = strtotime("$ano-$mes-$dia $tEntrada");

                        $tSaida = $rst_sel['usuario_TARDESAIDA'];
                        $D = strtotime("$ano-$mes-$dia $tSaida");

                        $sabadoEntrada = $rst_sel['usuario_sabadoe'];
                        $E = strtotime("$ano-$mes-$dia $sabadoEntrada");

                        $sabadoSaida = $rst_sel['usuario_sabados'];
                        $F = strtotime("$ano-$mes-$dia $sabadoSaida");
                  //  }
                ...
                if($stm->rowCount() > 0){
                        ...
                        unset($_SESSION['per402']);
                         ...
                         unset($_SESSION['per159']);
                         unset($_SESSION['per160']);

                        unset($_SESSION['per161']);
                        unset($_SESSION['per162']);
                        unset($_SESSION['per163']);

                        unset($_SESSION['per164']);
                ...
                foreach ($stm->fetchAll(\PDO::FETCH_OBJ) as $rst) {
                    ...
                    if($rst->tela_descricao == "242"){  $_SESSION['per242'] = $rst->tela_descricao; }// BLOQUERAR ACESSO A O.S
                      ...
                      if($rst->tela_descricao == "161"){ $_SESSION['per161'] = $rst->tela_descricao; }//bloqueia RESMO OS
                      if($rst->tela_descricao == "163"){ $_SESSION['per163'] = $rst->tela_descricao; }//BLOQUEIA ALTERAÇÃO CADASTRO CONSUMIDOR
                       if($rst->tela_descricao == "164"){ $_SESSION['per164'] = $rst->tela_descricao; }//BLOQUEIA NOVO MODELO/APARELHO O.S
                    ...
                    if($rst->tela_descricao == "159"){ $_SESSION['per159'] = $rst->tela_descricao; }//tipo garantia/oficina
                    if($rst->tela_descricao == "162"){ $_SESSION['per162'] = $rst->tela_descricao; }//BACKUP
            ...
            $v1 = "on";
                   ...
                   /*
                        ...
                        $status = 0;
                       if ($data >= $A && $data <= $B) $status = 1; 
                     if ($data >= $C && $data <= $D) $status = 1; 
                        if ($data >= $E && $data <= $F) $status = 1; 

                  
                            $horarioAtual = date("H:i:s", $data); // formato legível
                            
                        

                         //  $message =  "<strong>Acesso não permitido! Fora do horário de acesso.</strong>";

                        if($status == 0 ) { 
                                     $horarioAtual = date("H:i:s", $data); // formato legível
                                    $message = "⏰ Horário atual: <b>$horarioAtual</b><br>
                                                ❌ Acesso não permitido! Fora do horário de acesso.";
