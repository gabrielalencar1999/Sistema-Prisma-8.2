...
$sql = "SELECT * FROM info.consumidor WHERE  ind_situacao > 0 and ind_situacao <> 3  and ind_situacao <> 4  "; // 3 situacao cancelado 4 - desistencia |   autorizado > 0 
    ...
    while ($row = $result->fetch_assoc()) {
      ...
      $info = "Ref. ao mês 09/2025 ";
       ...
       // echo "<h3>Dados do banco: $banco</h3>";
        $REG  = 0; $VLROS = 0;
        $sqlParametro = "SELECT COUNT(SituacaoOS_Elx) AS REG FROM `$banco`.chamada where DATA_CHAMADA BETWEEN '2025-09-01' AND '2025-09-30' AND  SituacaoOS_Elx <> '10' ";
        ...
        if ($resParametro) {
            ...
            if($REG > $limitemaxos) {
                  echo "$idcliente - $banco  OS ABERTA $REG - ";
              $REG = $REG-$limitemaxos;
                  echo " ($REG) O.S adicionais<BR>";
