...
*/
		...
		$_SESSION['company'] = "PRISMA GESTÃO";
	 	...
	 	$style='<link href="css/styleCaixaBlue2.css" rel="stylesheet">';
	...
	//clinte novo pedido 
	$idcliente=  base64_decode($_GET['refidC']);	
	$idclienteRet = explode('|',$idcliente);

	if($idclienteRet[0] == ""){
		$idcliente = 1;
		$nomecliente = "CONSUMIDOR";
	}else{
		$idcliente = $idclienteRet[0];
		$nomecliente = $idclienteRet[1];
	}
	...
	//	$_SESSION['numberPedido'] = $idPedido;		
		...
		//cria pedido
		 ...
		 $_idfrefGO =   base64_encode("p-".$idPedido."-1-1");
		// $idcliente = $_refidcliente	;
		// $nomecliente = "CONSUMIDOR";

	}else{
		if($_numeropedido != ""  and  $_idfref[1] == "") {
			
			//$_numeropedido = $_SESSION['numberPedido'];

			$sql="select * from ".$_SESSION['BASE'].".saidaestoque where NUMERO = '".$_numeropedido."' and COD_Vendedor = '".$_SESSION["tecnico"]."'";
			$stm = $pdo->prepare($sql);
			$stm->execute();
			...
			if($_idfref[1] != "") {
				...
				LEFT JOIN ".$_SESSION['BASE'].".usuario ON COD_Vendedor = usuario_CODIGOUSUARIO 
				 where NUMERO = '".$_idfref[1]."'";
				$_numeropedido = $_idfref[1];
				//$_SESSION['numberPedido'] = $_numeropedido;
			}else{
				$sql="select * from ".$_SESSION['BASE'].".saidaestoque LEFT JOIN ".$_SESSION['BASE'].".usuario ON COD_Vendedor = usuario_CODIGOUSUARIO 
			...
			while($linha = $stm->fetch(PDO::FETCH_OBJ)){
				...
				$idcliente = $linha->CODIGO_CLIENTE;
				$nomecliente = $linha->CLIENTE;
				$_status = $linha->se_status;
				$_codigovendedor =  $linha->COD_Vendedor;
				$_nomevendedor =   $linha->usuario_APELIDO;
				$iDCAIXA =  $linha->num_livro;
				//buscar nf 
			}
		...
		//VERIFICAR SE 	em mais de um caixa habilitado 
		...
		if($totalcaixaativo > 1 ){
			if($iDCAIXA == "" ){
				...
				$sql="select Descricao from ".$_SESSION['BASE'].".livro_caixa_numero where Livro_Numero = '$iDCAIXA' ";
				//echo $sql;
				$stmcxsel = $pdo->prepare($sql);
				$stmcxsel->execute();
				foreach($stmcxsel->fetchAll(PDO::FETCH_ASSOC) as $value){
					$desccaixa = $value['Descricao'];
				}
			}
		...
		<div class="container-fluid">
			...
			<div class="row">
				...
				<div class="col-md-6 relogio">				
					<div id="labelcx"><span class="label label-table label-primary" style="font-size: 20px;"
					<?php 	if($_selecionarcx == 1 or $totalcaixaativo > 1) { ?>	onclick="_cxviewer()" <?php } ?>					><?=$desccaixa;?></span></div>
				</div>
			
				<div class="col-md-3 statusCaixa">
					<div id="situacaoCaixa" style="color:#000000; font-weight:300;">N° VENDA: <b style="color:#333;"><span class="label label-table label-inverse" style="font-size: 20px;"><?=$_numeropedido;?></span></b></div>
					<!---INPUT DE SITUAÇÃO DO CAIXA---->
					<input type="hidden" id="sitCx" name="sitCx" value="1" style="color:#000;">
					<div id="relogio" style="margin-top: 5px;"></div>	
				</div>
			</div>
			<br>
			<div class="row">
				<div class="col-md-6" style="height:87%">
						...
						<div id="listaItem">
							<table class="table">
								<tr>
									...
									<th class="lin" style="min-width: 100px ;" >TOTAL</th>
								</tr>
								<?php
									//busca valor desconto total da venda
									$sql = "select VL_DESCONTO_porc from ".$_SESSION['BASE'].".saidaestoque where NUMERO = '".$_numeropedido."'";
									$stm = $pdo->prepare($sql);
									$stm->execute();
									while($rst = $stm->fetch(PDO::FETCH_OBJ)){ 
										$descontoVenda = $rst->VL_DESCONTO_porc;
									}
									$sql = "select ITEM,CODIGO_FABRICANTE,DESCRICAO_ITEM,QUANTIDADE,VALOR_UNIT_DESC,VALOR_TOTAL
								...
								?>
										<tr>
											...
											<td style="text-align: center;"><b><?="R$ ".number_format($linha->VALOR_UNIT_DESC,2,',','.');?></b></td>
					...
					<?php  if($_SESSION['per252'] == '252') { //esconde valores ?> 
						<div class="col-sm-12 top valorT"> <span id="tt" style="color:#2FFF00;display:none;" ></span></div>			
						<?php						
					}else{ ?>
				...
				<div class="col-md-6" style="height:87%">
					<div class="row o2">
					     <?php 
								if($_SESSION['per115'] == '115') { //liberada todos almoxarifado seleção

									//VERIFICAR SE EXISTE DEFINIÇÃO ALMOXARIFADO PARA USUARIO 
									$sqluser = "Select au_almoxid,au_user,Codigo_Almox,Descricao FROM ".$_SESSION['BASE'].".almoxarifado_user 
									LEFT JOIN  ".$_SESSION['BASE'].".almoxarifado ON Codigo_Almox = au_almoxid
									WHERE au_user  = '".$_SESSION['tecnico']."' ";
													...
													<select name="_almox" id="_almox" class="form-control input-sm">									
														<?php
															foreach($stmuser->fetchAll(PDO::FETCH_ASSOC) as $value){
																$codigoSit = $value["Codigo_Almox"];
																$descricaoSit = $value["Descricao"];
                               	...
                               	}else{ ?><input type="hidden" name="_almox" id="_almox" class="form-control input-sm" value= "1">
                                    <?php
                                
							}
								
							if($situacaoPedido == '3' or $situacaoPedido == '9') {
								//nao libera para pesquisar e inluir
							}else{
							?>
					...
					<div class="row o3 top">
						...
						<div class="col-sm-1" style="padding:1px;">
							<input type="text" id="qtde" name="qtde" placeholder="0" class="form-control" value="1">
						</div>
						<div class="col-sm-2 default" style="text-align:right;">
							<b>CLIENTE: </b>
						</div>
						<div class="col-sm-7 default" style="text-align:center; border-bottom:1px solid #C1C1C1;">
							<input type="hidden" id="idcliente" name="idcliente" placeholder="0" class="form-control" value="<?=$idcliente;?>">
							<span id="desccliente" style="opacity:0.6;"><?=$nomecliente;?></span>
					...
					if($situacaoPedido == '3' or $situacaoPedido == '9') {
						if($situacaoPedido == '3') { $color ='#81c868'; } else{ $color ='#f05050';}	  ?>
					<div class="row " style="background-color: <?=$color;?>;  ">
							<div class="col-xs-12  top" style="margin-bottom: 10px;">								
								<span style="color:#00A8E6;"><b style="color:#FFF">Pedido: <?=$sistuacaoDesc;?> 																				  
							</div>
					</div>
					<div class="row " style="background-color: #515050; padding:15px"  >
							...
							<?php 
									if($SAIDA_NFE == 0){ ?>		
									<div class="col-xs-4  top">
										<button type="button" class="btn btn-default btn-block" style="padding:12px;" onclick="gerarNFCE()"> <b>Gerar NFC-e</b></button>
									</div>
									<?php } 
									?>
					</div>
					<div class="row " style="background-color: #515050;  "   >
						<div class=" col-xs-3  top">	
							<span style="color:#00A8E6;"> Vendedor:  </span>
								<?php if($_SESSION['per253'] == '253'){ ?>
								<Select  class="form-control" id="_vendedor"  name="_vendedor" >
														  ...
														  <?php  
															  ...
															  foreach ($result as $row) {
																  ...
																  ?>   
																	  <option value="<?php echo "$codigo"; ?>" <?php if ( $codigo == $_codigovendedor) { ?>selected="selected" <?php } ?>> <?php echo "$descricao"; ?></option>
																	  <?php
																  }
																  ?>   
														  </Select> 
														  ...
														  <?php } else { ?>														
																 <b style="color:#FFF;"><?=$_nomevendedor;?></b> 
														  <?php }  ?>	
							</div>
							<?php if($_SESSION['per253'] == '253'){ ?>
							<div class=" col-xs-3" style="margin-top: 40px;" id="salvarparametros">	
							...
							<?php }
							?>
						<div class="row " style="background-color: #515050;  padding-bottom:30px" style>  
							<div class="col-xs-12 top ">								
								<span style="color:#00A8E6;">Observação:  
								<textarea name="obspedido" id="obspedido" rows="2" class="form-control input-sm" ><?=$OBSERVACAO;?> </textarea></span>
					...
					<div class="row " style="background-color: #515050;    padding:15px">
						<div class="col-sm-12 " >
							...
							<div class="col-xs-3"><button class="btn btn-white btn-block" onclick="Entrada()"  ><b>ENTRADA R$<BR></button></b></button></div>
							...
							<div class="col-xs-3 top"><button  class="btn btn-white btn-block" id="cliente" onclick="buscaCliente()" data-toggle="modal" data-target="#modal-cliente"><b>CLIENTE</button></b></button></div>
...
<!--
							...
							<div class="col-xs-12 top">								
								<span style="color:#00A8E6;">Atalhos: <b style="color:#FFF">(ENTER)</b> Código /Add item <b style="color:#FFF"></b> </span>
							</div>
							<div class="col-xs-3 top">
							<span style="color:#00A8E6;"><b>STATUS</b> </span>
							<Select  class="form-control" id="_status"  name="_status" >
                                                            ...
                                                            <?php  
                                                                    ...
                                                                    $result = $consulta->fetchAll();
                                                                        foreach ($result as $row) {
                                                                            ?><option value="<?=$row["stavenda_id"];?>" <?php if($row["stavenda_id"] == $_status) { echo "selected"; } ?>><?=($row["stavenda_desc"]);?></option><?php
                                                                        }
                                                                ?>  
                                                            </Select>
							</div>
							<div class="col-xs-3 top">		
											<?php 
												if($_SESSION['per250'] == '250') { //liberada todos almoxarifado seleção													
													?>				
										
												<span style="color:#00A8E6;"><b>BAIXAR ESTOQUE</b> </span>
													<select name="_BX" id="_BX" class="form-control input-sm">
														<option value="1">SIM</option>
														<option value="0">NÃO</option>
													</select>
													<?php
													}
													else{ ?><input type="hidden" name="_BX" id="_BX" class="form-control input-sm" value= "1">
														<?php
													}
													?>													
		...
		<div class="modal fade bs-example-modal-lg" id="finalizarVenda" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" data-backdrop="static">
		  <div class="modal-dialog modal-lg" role="document">
			...
			<!-- MODAL -->
			<div class="modal fade bs-example-modal-lg" id="modal-cliente" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" data-backdrop="static">
		  <div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<div class="col-xs-8"><h5>CLIENTE</h5></div>
					<div class="col-xs-4" id="fechar_pgto"><button type="button" class="close" data-dismiss="modal" style="margin-top:4px;"><span aria-hidden="true">×</span></button></div>
					
				</div>
				
			  <div class="modal-body" id="respBuscaCONSUMIDOR" >
	 ...
	 <div class="modal fade"  id="custom-modal-caixa" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" data-backdrop="static" >
            <div class="modal-dialog " >
                <div class="modal-content">
                        <div class="modal-header">  
                        ...
                        <div class="row">
                                <div class="col-sm-12">
                                    <div class="card-box" >                                       
                                            <div class="row">
                                                <div class="col-md-12" > 
                                              <div style="text-align: center;margin-bottom: 20px"> Selecione o caixa  </div>
											  <?php 
													if($_selecionarcx == 1 or   $totalcaixaativo > 1) {													
															foreach($stmcx->fetchAll(PDO::FETCH_ASSOC) as $value){
																
																?>
																<button type="button"  class="btn btn-block btn-info btn-lg waves-effect waves-light" style="margin-bottom: 20px;"  onclick="_btSelcx(<?=$value['Livro_Numero'];?>);"><i class="ion-ios7-cart"></i> <?=$value['Descricao'];?></button>
															<?php
														}
														}
...
<!---FULLSCREEN FIM-->
<script>
	//verificar se $_selecionarcx  mais de um caixa


	   document.addEventListener('DOMContentLoaded', function () {
				const selecionarCx = <?= json_encode($_selecionarcx) ?>;
				const TTCx = <?= json_encode($totalcaixaativo) ?>;
				if (selecionarCx === 1 && TTCx > 1) {
					$('#custom-modal-caixa').modal({
						backdrop: 'static',
						keyboard: false
					});
				}
    ...
    function myTimer() {
       ...
       if (navigator.userAgent.toLowerCase().indexOf('firefox') > -1) {
          ...
          document.getElementById("relogio").innerHTML = dataHoje+" "+displayDate;
    }
				...
				if(key == '13' && cod != "" && sitVenda == '1'){	
					if(cod == '1'){
						...
						$("#sitCx").val('5');
								...
								$.post('acaoCaixa.php',{almox:almox ,cod:cod , qtde:qtde , acao:'lerCodigoBarra',ref:$("#_ref").val()}, function(resp){
	...
	function finalizaVenda(){
		...
		$("#sitCx").val('3');	
		var almox = $("#_almox").val();
		 $.post('acaoCaixa.php',{almox:almox,acao:'validaestoque',ref: $("#_ref").val(),bx: $("#_BX").val()}, function(resp){
			var retorno = $.trim(resp);

			if(retorno == ""){
				$("#finalizarVenda").modal();
				$.post('acaoCaixa.php',{almox:almox,acao:'finalizaPagamento',ref: $("#_ref").val(),_status: $("#_status").val(),obs: $("#obspedido").val(),bx: $("#_BX").val()}, function(resp){
				var retorno = $.trim(resp);
				$('#divPag').html(retorno);	
			});
			}else{
				
				$.post('acaoCaixa.php',{almox:almox,acao:'validaestoqueViewer',ref: $("#_ref").val(),bx: $("#_BX").val()}, function(resp){
					var retorno = $.trim(resp);
					$('#listaItem').html(retorno);	
								//alert(retorno);
								//location.reload();
		...
		function _btSelcx(_cx){
		
		 $.post('acaoCaixa.php',{acao:'selcx',idsel: _cx,ref: $("#_ref").val()}, function(resp){		
			$('#labelcx').html(resp);
			$("#custom-modal-caixa").modal('hide');		
		});
		
	}
		function _cxviewer(_cx){
				$("#custom-modal-caixa").modal('show');	
	...
	function finalizaSalva(_ped){
		$('#divSalvar').html('' +
		 ...
		 $.post('acaoCaixa.php',{acao:'finalizaSalva',ref:$("#_ref").val(),_status: $("#_status").val(),obs: $("#obspedido").val(),bx: $("#_BX").val()}, function(resp){
			var retorno = $.trim(resp);
	...
	function gerarNFCE(){
		...
		$.post('acaoCaixa.php',{acao:'confGerarNFCE',ref:$("#_ref").val(),_status: $("#_status").val(),obs: $("#obspedido").val(),bx: $("#_BX").val()}, function(resp){
		   var retorno = $.trim(resp);
		   $('#divSalvar').html(retorno);	
	   });
	...
	function _btSalvar(){	
		$.post('acaoCaixa.php',{acao:'btSalvar',ref: '<?= $_numeropedido;?>'}, function(resp){
		   var retorno = $.trim(resp);
		   $('#retacao').html(retorno);	
	   });	   
   }

   function _salvarparametros(){	
	$('#salvarparametros').html('' +
						            '<div class="bg-icon pull-request">' +
                							'<img src="../assets/images/loading.gif" class="img-responsive center-block" width="50".">' +                							
            								'</div>');
		$.post('acaoCaixa.php',{acao:'btSalvararametros',ref:$("#_ref").val(),vend:$("#_vendedor").val()}, function(resp){
		   var retorno = $.trim(resp);
		   $('#salvarparametros').html(retorno);
	   });	   
   ...
   function _btCancelar(){	
						$('#divSalvar').html('' +
						            '<div class="bg-icon pull-request">' +
                							'<img src="../assets/images/loading.gif" class="img-responsive center-block" width="100" alt="imagem de carregamento, aguarde.">' +
                							'<h2 class="text-center">Aguarde, cancelando venda...</h2>'+
            								'</div>');
		$.post('acaoCaixa.php',{acao:'btCancelar',ref:$("#_ref").val()}, function(resp){
		   var retorno = $.trim(resp);
		   $('#divSalvar').html(retorno);	
	   });	   
   }
   ...
   function _btOrcamento(_ped){	
		$.post('acaoCaixa.php',{acao:'btOrcamento',ref: $("#_ref").val()}, function(resp){
		   var retorno = $.trim(resp);
		  $('#retacao').html(retorno);	
	   });	  
   ...
   function _btPedido(_ped){	
		$.post('acaoCaixa.php',{acao:'btPedido',ref: $("#_ref").val()}, function(resp){
		   var retorno = $.trim(resp);
		  $('#retacao').html(retorno);	
	   });	  
	   var $_keyid = "_PDV00006";
								$('#_keyform').val($_keyid);
								var dados = $("#form1 :input").serializeArray();
								dados = JSON.stringify(dados);
								$('#_printviewer').html("");
   ...
   function _EmitirNFce(){	
	$('#retacao').html('' +
						            '<div class="bg-icon pull-request">' +
                							'<img src="../assets/images/loading.gif" class="img-responsive center-block" width="100" alt="imagem de carregamento, aguarde.">' +
                							'<h2 class="text-center">Aguarde, carregando dados...</h2>'+
            								'</div>');
		$.post('acaoCaixa.php',{acao:'EmitirNFce',ref: $("#_ref").val()}, function(resp){
		   var retorno = $.trim(resp);
		  $('#retacao').html(retorno);	
	   });	  
	...
	function add_produto(){
		...
		if(valorProduto != "" && valorProduto > 0){
			$.post('acaoCaixa.php',{valorProduto:valorProduto , acao:'ProdutoNaoCadastrado', qtde:qtde , cod:cod,ref:$("#_ref").val()}, function(resp){
				var retorno = $.trim(resp);
				$('#listaItem').html(retorno);	
				$("#codigoBarras").val("");
				$("#qtde").val("");
	...
	function fecharModal(){
		...
		$("#modal-reabrirVenda").modal('hide');
		 //status do caixa
		$("#sitCx").val('1');
		$('#fechar_pgto').html('<button type="button" class="close" onclick="fecharModal()" style="margin-top:4px;"><span aria-hidden="true">×</span></button>');
	}
	...
	function localizarCliente(){
		var _desc = $("#_desccli").val();
		var _ordem = $("#ordem").val();

		document.getElementById('ordem').value == "CGC_CPF";

		$.post('acaoCaixa.php',{descricao:_desc, ordem:_ordem , acao:'localizarCliente'}, function(resp){
			var retorno = $.trim(resp);
			$("#_resultclinew").html(retorno);
		});
	}

	function selCliente(idCliente,nomeCliente){
		$.post('acaoCaixa.php',{codigocli:idCliente, nomecli:nomeCliente , acao:'alteraConsumidor',ref: $("#_ref").val()}, function(resp){
		
		});
		$("#idcliente").val(idCliente);
		$("#desccliente").html(nomeCliente);
	...
	function _FinalizarVendaCartao (tipo){
		...
		dialog.find('.bootbox-body').html('<i class="fa fa-spin fa-cog"></i> Em processamento, aguarde...');
						$.post('acaoCaixa.php',{almox:almox,valorPago:valorPago, selpag:selpag , parpag:parpag, valorApagar:valorApagar, troco:troco, tipo:tipo, acao:'fimPagamento',ref: $("#_ref").val(),bx: $("#_BX").val()}, function(resp){
							var retorno = $.trim(resp);
	...
	function fimCartao(tipo){
			...
			var almox = $("#_almox").val();
							...
							aguarde(tipo);
											...
											$.post('acaoCaixa.php',{almox:almox, tipo:tipo,acao:'verPagamentocartao',ref: $("#_ref").val()}, function(resp){
												...
												//	$.post('acaoCaixa.php',{valorPago:valorPago, selpag:selpag , parpag:parpag, valorApagar:valorApagar, troco:troco, tipo:tipo, acao:'fimPagamento',ref: $("#_ref").val()}, function(resp){
													$.post('acaoCaixa.php',{almox:almox,acao:'fimPagamento',ref: $("#_ref").val(),bx: $("#_BX").val()}, function(resp){
														var retorno = $.trim(resp);
													//	dialog.find('.bootbox-body').html(retorno);    ;
														//dialog.modal('hide');
														$('#divPag').html(resp);
	...
	function fim(tipo){
		...
		*/
		...
		var vdescontogeral = $("#descontogeral").val();
		...
		if(vdescontogeral == ""){
			...
			vdescontogeral = parseFloat(vdescontogeral.replace('.','').replace(',','.'));
		}

		var ventradageral = 0;
		var ENTRADA = $("#entradageral").val();
		if(ENTRADA == ""){
			ventradageral = 0;
		}else{
			ventradageral = parseFloat(ENTRADA.replace('.','').replace(',','.'));
		}
		
		
		
		var valorApagar = parseFloat(valorTotal.replace('.','').replace(',','.'));
		valorApagar = valorApagar + vfrete - vdescontogeral-ventradageral;
		...
		if(valorApagar <=  valorPago && valorPago > 0 || ventradageral  > 0){
				...
				if(dinheiro == "SIM" && liberaTroco == "SIM"){
					...
					$.post('acaoCaixa.php',{almox:almox,empresa:vempresa,descontogeral:vdescontogeral,frete:vfrete,valorPago:valorPago, selpag:selpag , parpag:parpag, valorApagar:valorApagar, troco:troco, entrada:ventradageral, tipo:tipo, acao:'verPagamento',ref: $("#_ref").val(),pagamentos:pagamentos}, function(resp){
											
						if(resp == ''){  
							// finaliza venda
							//GERA REGISTRO NO CONTAS A RECEBER
							//dialog.find('.bootbox-body').html('<i class="fa fa-spin fa-cog"></i> Em processamento, aguarde...');
							$.post('acaoCaixa.php',{almox:almox,empresa:vempresa,descontogeral:vdescontogeral,frete:vfrete,valorPago:valorPago, selpag:selpag , parpag:parpag, valorApagar:valorApagar, troco:troco, entrada:ventradageral, tipo:tipo, atendente:atendente, acao:'fimPagamento',ref: $("#_ref").val(),bx: $("#_BX").val()}, function(resp){
								var retorno = $.trim(resp);
	...
	function Entrada(){
		var retorno = "";
				$.post('acaoCaixa.php',{acao:'valorEntradaRet',ref: $("#_ref").val()}, function(resp)
								{																																		
	...
	function verificaValorEntrada(){		
		var valor = $("#entradaValor").val();	
			
								//location.reload();
								$.post('acaoCaixa.php',{acao:'valorEntrada', entradaValor:valor ,  idcliente:$("#idcliente").val(), idPgto:$("#selpagEntrada").val(), ref: $("#_ref").val()}, function(resp)
								{	
								var retorno = $.trim(resp);								
								bootbox.hideAll();							
								}
	...
	function soma() { 
		...
		var descGeral = $("#descontogeral").val();
		if(descGeral == ""){
			descGeral = 0;
		}else{
			descontogeral = parseFloat(descGeral.replace('.','').replace(',','.'));
		}
		var ventradageral = 0;
		var ENTRADA = $("#entradageral").val();
	  ...
	  var troco = valorTotal - (totalVenda+valorFrete-descontogeral-ventradageral);
	  if(troco < 0){
		  troco = 0;
	  }
	  $("#trocoC").val(troco);
	  ...
	  var faltaPagar2 = (valorFrete+totalVenda-descontogeral-ventradageral) - valorTotal; //entradageral
