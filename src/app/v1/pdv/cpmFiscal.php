	...
	if ($stm->rowCount() > 0 ){
		foreach($stm->fetchAll(PDO::FETCH_ASSOC) as $rst){	
			...
			if($estado == "RS") {
				...
				if($estado == "MG") {
				$_linkwebservice = "https://portalsped.fazenda.mg.gov.br/portalnfce/sistema/consultaarg.xhtml";
			}
				if($estado == "CE") {
				$_linkwebservice = "http://nfce.sefaz.ce.gov.br/pages/consultaNota.jsf";
			}
...
$sql = ("SELECT Valor_Frete,OBSERVACAO,SAIDA_EMPRESA FROM ".$_SESSION['BASE'].".saidaestoque
WHERE NUMERO = '$pedido' ");
$stm = $pdo->prepare($sql);
$stm->execute();
				...
				<?php 	} ?>
						...
						<tr>
							...
							<td colspan="2" style="text-align:right; font-size:12px;">Valor Pago</td>
									...
									<?php 
										$sql="select nome,spgto_entrada,spgto_valor from ".$_SESSION['BASE'].".saidaestoquepgto left join ".$_SESSION['BASE'].".tiporecebimpgto on id=spgto_tipopgto where spgto_numpedido = '$pedido' group by nome,spgto_valor,spgto_entrada";
										$stm = $pdo->prepare($sql);
										$stm->execute();
										foreach($stm->fetchAll(PDO::FETCH_ASSOC) as $pagamento ){
											...
											?>
												<tr>
													...
													<td style="font-size:12px; text-align:right;"><?=number_format($pagamento['spgto_valor'],2,',','.');?></td>
												</tr>
											<?php
										}
										if($troco > 0) { 
	...
	<tr>
		<td class="center">
			...
			//	$tempDir = "../qrcodes/";
				...
				$urlRelativeFilePath = "/app/v1/pdv/qrcodes/".$fileName;
